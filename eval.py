...
from transformers import (BertModel, BertTokenizer,
                          ...
                          RobertaForSequenceClassification, AutoModel, RobertaForMaskedLM)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', default=79, type=int)
    ...
    test_tree_file_path = 'roberta_dist_test_{}/{}_tree.pkl'.format(args.dataset, args.layer)
    test_img_tree_file_path = 'roberta_dist_img_test_{}/{}_tree.pkl'.format(args.dataset, args.layer)
    ...
    test_dataloader = DataLoader(dataset=test_datalset,
                                 batch_size=32,
                                 shuffle=False)
    for pth in os.listdir('out_15'):
        if not pth.endswith('.pth'):
            continue
        model.load_state_dict(torch.load(os.path.join('out_15',pth)))
        model.eval()
        encoder.eval()
        pred, gt = [], []
        with torch.no_grad():
