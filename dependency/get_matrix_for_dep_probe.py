...
def get_dep_matrix_new(args, model, tokenizer, dataset):
    ...
    if args.cuda:
        model.to('cuda')
    for sentence, indexed_tokens, mapping in tqdm(dataset):
...
#        print(sentence)
 ...
 #       print(mapping)
        ...
        for k, one_layer_matrix in enumerate(all_layers_matrix_as_list):
            ...
            out[k].append((sentence, tokenized_text, init_matrix, mapping))
    for k, one_layer_out in enumerate(out):
        k_output = args.output_file.format(str(k))
        with open(k_output, 'wb') as fout:
            pickle.dump(one_layer_out, fout)
